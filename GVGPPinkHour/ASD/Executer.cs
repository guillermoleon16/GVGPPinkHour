...
namespace GVGPPinkHour
{
    public class Executer
    {
        ...
        private void ReinforcePaths(List<AptitudeState> aptitudes, int target)
        {
            ...
            foreach(AptitudeState ap in optimalPath)
            {
                double[] tempout = NetworkBank[target].Compute(ap.State.Board);
                int act = 0;
                for (int m = 1; m < tempout.Length; m++)
                {
                    if(tempout[act] < tempout[m])
                    {
                        act = m;
                    }
                }
                if(ap.State.Links[act] == null)
                {
                    cont++;
                }
            }
            Console.WriteLine("Error percentage = " + (double)cont/(double)aptitudes.Count);
        }
    }
}
